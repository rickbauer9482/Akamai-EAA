...
client = Client(api_key='XXXXXXXXX', secret_key='YYYYYYYYYY')
...
for line in fl:
    ...
    app.connectors.add(name=fh[6])
    app.connectors.add(name=fh[7])

    # Add App to a Category
    app.set_app_category('VDI and TermSvr')

    # Configure the IDP and Directory
    app.set_idp(name=fh[8])
    app.directories.add(name=fh[9])
    app.groups.add(directory_name=fh[9], group_names=[fh[10]])

    # Add users to the Overlay group
    spgi_dr = directories.get(name=fh[9])
    spgi_usr = spgi_dr.users.get(name=fh[11])
    spgi_grp = spgi_dr.groups.get(name=fh[10])
